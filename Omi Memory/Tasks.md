# Tasks

_Last updated: 2026-05-26_

---

## Notion Tasks
_Synced: 2026-05-24 21:30_

### 🔴 Doing
- [ ] Give Editor All the assets
- [ ] Improve Drone Videography
- [ ] Improve Videography & Editing
- [ ] Learn how to use Dji mini 5 Drone _(High)_
- [ ] Improve Videography & Editing _(due 2026-01-28)_

### 🟡 To-Do
- [ ] Break down Escalation Media Website then get on a call. Then contact web designer. _(High)_
- [ ] Create Weekly Schedule _(High)_
- [ ] Create Delivery SOP's _(High)_
- [ ] Reschedule Line Of Credit Application With The bank and get the proper documents _(High)_
- [ ] Send voice memo to Helen
- [ ] Figure out all legal information about going to miami
- [ ] Get Boating License
- [ ] Check out contract that chris sent
- [ ] Answer Question Lisa sent for Lawyer meeting
- [ ] Organize Resimate files
- [ ] Send Raw Footage to the Home Building Company (Person gave you a card)
- [ ] Setup Greening Homes in Click up
- [ ] Deep Dive Claude Work Flow
- [ ] Schedule in Updated Resimate Shoots
- [ ] Split / Organize the 4 prospect videos
- [ ] Create ALVION website
- [ ] Make a Escalation Media Edit (Talking Video)
- [ ] Create a agent for social media competitor and content research.
- [ ] Improve Real-Estate color grade
- [ ] Make a Reel for Prospect Lab (Malechie not making the jump to making the jump.)
- [ ] Drone Shot of the Pear in Burlington
- [ ] Study 910 Academy course
- [ ] Send Raw Footage to Volpe Financial
- [ ] Start Building Out Alvion content Style
- [ ] Start Learning Apple Motions
- [ ] Claude Cowork Learning
- [ ] SMMA Study (Branding)

### ⏳ Waiting
- [ ] Find videographer (Start with Indeed Postings) _(High)_
- [ ] Oakville Chamber Edit _(High)_
- [ ] Send Reference Video / Raw Footage to Potential Video Editor.

---

## 2026-05-25
[[Omi Collection/2026/05/25/2026-05-25|Source]]

- [ ] Review revised contract (videographer / Pixel City Visuals)
- [ ] Send contract for review (videographer)
- [ ] Send WhatsApp number to videographer
- [ ] Send drone registration and pilot certificate links to videographer
- [ ] Schedule weekly Monday noon check-ins with videographer
- [ ] Confirm whether you can color grade Canon C-Log footage
- [ ] Adjust Joe calendar event to end at 10:00 PM
- [ ] Update the project list
- [ ] Do research on ancient history

---

## 2026-05-23
[[Omi Collection/2026/05/23/2026-05-23|Source]]

- [ ] Create AI accountant project in Obsidian
- [ ] Set up editor for Greening Homes ClickUp project

---

## 2026-05-22
[[Omi Collection/2026/05/22/2026-05-22|Source]]

- [ ] Create group chat with the new videographer after onboarding
- [ ] Send project three footage to client for script approval
- [ ] Send Greening Homes photos to client
- [ ] Get Greening Homes client on a call to define project direction
- [ ] Send Helen a contract

---

## 2026-05-21
[[Omi Collection/2026/05/21/2026-05-21|Source]]

- [ ] Get the infinite question glitch working for the mid network
- [ ] Network with people who have strong connections in the Zoom world
- [ ] Set up Zapier integration with WhatsApp and Claude
- [ ] Set up Zapier integration with Obsidian

---

## 2026-05-20
[[Omi Collection/2026/05/20/2026-05-20|Source]]

- [ ] Create profiles for Ryan, Maverick, Ahmed, and Chris
- [ ] Finalize calendar and connect it to Claude
- [ ] Send Project Three revisions to editor
- [ ] Sort Helen's contract
- [ ] Fix Omi screen capture
- [ ] Build daily task and calendar UI
- [ ] Review contract created by Chris
- [ ] Create contract for Helen
- [ ] Organize Resume files
- [ ] Organize Prospect Labs file
- [ ] Set up Grady Homes ClickUp file

---

## 2026-05-17
[[Omi Collection/2026/05/17/2026-05-17|Source]]

_No action items recorded.
